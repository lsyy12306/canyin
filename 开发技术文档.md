...
> ⚠️ 修正说明：本项目 ORM 使用**同步** `create_engine` + `SessionLocal`（见 `app/database.py`），
> 因此生产 PostgreSQL 连接串必须用**同步**驱动 `postgresql+psycopg://...`；
> 下方以 `weihe` 库名与 §12.2 一致（旧稿 `weihe_db` 改为 `weihe`）。
> 另：实际编排见仓库根 `docker-compose.yml`（含 admin 后台、uploads 数据卷、健康检查），比下方示意更完整。

```yaml
# docker-compose.yml（示意，完整版以仓库根文件为准）
services:
  db:
  ...
  backend:
    build: ./backend
    environment:
      # 同步驱动！与同步 SQLAlchemy 引擎匹配
      DATABASE_URL: postgresql+psycopg://weihe:weihe_pass@db/weihe
    ports:
      - "8000:8000"
    depends_on:
      - db
...
| 变量 | 说明 | 示例 |
|---|---|---|
| `DATABASE_URL` | PostgreSQL 连接串（**同步**驱动） | `postgresql+psycopg://user:pass@db:5432/weihe` |
  ...
  test:
    ...
    steps:
      ...
      - name: Backend import check
        # 项目暂无 pytest 用例，这里做导入冒烟校验（见 .github/workflows/deploy.yml）
